...
author: "Amy Fox"
vignette: >
  %\VignetteIndexEntry{Cytotypr Example: Reading in Data}
...
library(kableExtra)
dtTemplate %>%
   kable(align = c("l", "c", "c", "c"), caption = "Gating Template") %>%
...
```{r}
# check_flowjo <- function(df, marker, file) {
#   
#   df %>%
#   filter({{marker}} == 1) %>%
#   group_by(filename) %>%
#   mutate(sum_percentage  = sum(percentage)) %>%
#   select({{marker}}, filename, sum_percentage) %>%
#   unique()
# }


check_flowjo(all_fe, CD3)
